...
#ifdef __STDC__
static void get_raw(FILE *infile, char *inname, int  **a, int  nx, int ny, int  swap);
static void get_fits(FILE *infile, char *inname, FILE *outfile, int  **a, int  *nx, int *ny);
...
#ifdef __STDC__
extern void get_data(FILE *infile[2], char *inname[2], FILE *outfile, int  **a, int  *nx, int *ny, char *format)
#else
extern void get_data(infile,inname,outfile,a,nx,ny,format)
...
#endif
{
	 ...
	 */
	(void) fclose(infile[0]);
	if (strcmp(format,"hhh") == 0 && infile[1] != infile[0])
		(void) fclose(infile[1]);
}

#ifdef __STDC__
static void get_raw(FILE *infile, char *inname, int  **a, int  nx, int ny, int  swap)
#else
static void get_raw(infile,inname,a,nx,ny,swap)
...
static void get_fits(FILE *infile, char *inname, FILE *outfile, int  **a, int  *nx, int *ny)
#else
static void get_fits(infile,inname,outfile,a,nx,ny)
FILE *infile;	/* input file pointer					*/
char *inname;	/* input file name						*/
...
static void get_hhh(FILE *infile[2], char *inname[2], int  **a, int  *nx, int *ny)
#else
static void get_hhh(infile, inname, a,nx,ny)
FILE *infile[2];	/* input file pointers					*/
char *inname[2];	/* input file names						*/
int  **a;			/* pointer to image array (returned)	*/
int  *nx, *ny;		/* image is NX x NY						*/
