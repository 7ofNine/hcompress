...
#ifdef __STDC__
static void xshuffle(int a[], int nx, int ny, int nydim);
static void yshuffle(int a[], int nx, int ny, int nydim);
#else
static void xshuffle();
static void yshuffle();
#endif

#ifdef __STDC__
extern void htrans(int a[], int nx, int ny)
#else
extern void htrans(a,nx,ny)
int a[];
int nx, ny;
#endif
{
...
static void xshuffle(int a[], int nx, int ny, int nydim)
#else
static void xshuffle(a,nx,ny,nydim)
...
static void yshuffle(int a[], int nx, int ny, int nydim)
#else
static void yshuffle(a,nx,ny,nydim)
int a[];				/* array to shuffle					*/
int nx;					/* number of elements in column		*/
int ny;					/* number of elements in row		*/
int nydim;				/* actual length of row in array	*/
	...
	for (j=1; j<nx; j++) {
		if (flag[j]) {
			...
			if (j != k) {
				...
				if (j != k) {
					fprintf(stderr, "error: yshuffle failed!\nj=%d k=%d\n", j, k);
					exit(-1);
				}
			}
		}
